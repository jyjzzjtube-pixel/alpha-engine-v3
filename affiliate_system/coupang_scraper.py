...
class CoupangScraper:
    ...
    def scrape_and_link(self, url: str) -> Product:
        ...
        try:
            ...
            logger.warning(f"웹 스크래핑 실패: {e}")

        # 2차: 스크래핑 실패 시 URL에서 키워드 추출 후 검색 API 폴백
        if not product or not product.title:
            logger.info("웹 스크래핑 실패 — 상품 검색 API 폴백")
            # URL에서 상품 ID 추출 또는 기본 검색
